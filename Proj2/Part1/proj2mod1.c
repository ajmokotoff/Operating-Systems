...
asmlinkage long new_sys_open(const char *path, int flags, mode_t mode) {
  int user = current_uid().val;

  if (user >= 1000) {
    printk(KERN_INFO "User %d opened file: %s\n", user, path);
  }
  return ref_sys_open(path, flags, mode);
}
...
asmlinkage long new_sys_close(int fd) {
  ...
  if (user >= 1000) {
    printk(KERN_INFO "User %d closed file descriptor: %d\n", user, fd);
  }
